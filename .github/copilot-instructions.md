...
| File | Lines | Contents |
|---|---|---|
| `03-MetricsHashKey.ps1` | ~117 | `$script:AppVersion` (currently `2.8.0`), metrics hash key persistence, `Get-HashedDomain`, `Handle-MetricsRequest` |
...
| `16c-NameserverChecks.ps1` | ~330 | Per-nameserver TXT consistency check behind `/api/nameservers`, which explains the "all nameservers respond but TXT/SPF looks missing" class of problem (the `zenithbank.com` case where authoritative nameservers serve **divergent** TXT sets and one host fails to resolve). `Invoke-RawDnsTxtQuery` is a self-contained cross-platform raw DNS TXT client over `System.Net.Sockets` (UDP send + TCP-on-truncation / UDP-timeout fallback) so it works on Linux PowerShell 7 where `Resolve-DnsName` does not exist. **GOTCHA:** PowerShell's `-shl` on a `[byte]` truncates back to 8 bits (`[byte]218 -shl 8 == 0`), so every 16-bit DNS field (transaction id, ANCOUNT, TYPE, RDLENGTH, TCP length prefix) MUST cast the high byte to `[int]` before shifting — otherwise reads silently collapse to the low byte. `Get-AuthoritativeNameserverHosts` discovers NS hostnames via DoH JSON (`Resolve-DohName`'s ValidateSet excludes NS), walking the queried name then its parents/registrable apex. `Resolve-NameserverPublicIps` resolves each NS to IPs via `ResolveSafely`/`Get-DnsIpString` and refuses any non-public IP via `Test-IsPublicIpAddress` (from `16b-WebsiteProbe.ps1`) as an SSRF guard. `Get-NameserverTxtStatus` orchestrates the bounded fan-out (`ACS_NAMESERVER_PROBE_MAX`, `ACS_NAMESERVER_PROBE_TIMEOUT_MS`, opt-out `ACS_DISABLE_NAMESERVER_PROBE`) and returns a stable payload with a `consistencyState` (`consistent`/`inconsistent`/`partial`/`unknown`/`none`) plus per-nameserver TXT sets and SPF/ACS presence flags. The four top-level functions are registered in `22-RunspaceSetup.ps1`; the nested socket/parse helpers inside `Invoke-RawDnsTxtQuery` are not. The rendered `card-nameservers` (with its collapsible `toggleNameserverDetails` panel) is captured by `screenshotPage`. |
