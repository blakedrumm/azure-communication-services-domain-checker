...
2. **Server mode** (default): starts an HTTP listener, serves the SPA UI, and exposes routes such as `/`, `/index.html`, `/dns`, `/api/base`, `/api/mx`, `/api/records`, `/api/whois`, `/api/dmarc`, `/api/dkim`, `/api/cname`, `/api/reputation`, `/api/website`, `/api/metrics`, `/api/auth/event`, `/terms`, and `/privacy`.
...
| File | Lines | Contents |
|---|---|---|
| `03-MetricsHashKey.ps1` | ~117 | `$script:AppVersion` (currently `2.6.0`), metrics hash key persistence, `Get-HashedDomain`, `Handle-MetricsRequest` |
...
| `16b-WebsiteProbe.ps1` | ~470 | Security-hardened website reachability / parked-page probe behind `/api/website`: `Test-IsPublicIpAddress` (IPv4 + IPv6 SSRF guard, unwraps IPv4-mapped IPv6 so `::ffff:10.0.0.1` cannot bypass it), `Test-WebsiteHostIsPublic` (resolves the host and refuses unless **every** resolved address is public), `Get-WebsiteSnapshot` (creates an `HttpClient` with `AllowAutoRedirect=$false`, re-validates **each** redirect hop's scheme/port/public-IP, reads the body under a hard byte cap with a whole-operation `CancellationTokenSource`, then extracts `<title>`/meta-description/visible-text and neutral parked/placeholder/default-server markers), `Format-WebsiteText`, and `Get-WebsiteProbeStatus` (orchestrates https→http × apex→www candidates and returns a strictly **neutral/factual** summary token — `Reachable`/`PlaceholderContent`/`ServerError`/`ClientError`/`Unreachable`/`Disabled`). Output deliberately makes **no** abuse/intent judgement. Tunable via `ACS_DISABLE_WEBSITE_PROBE`, `ACS_WEBSITE_PROBE_TIMEOUT_SEC`, `ACS_WEBSITE_PROBE_MAX_BYTES`, `ACS_WEBSITE_PROBE_MAX_REDIRECTS` (read inline, mirroring the RBL check). All five functions are registered in `22-RunspaceSetup.ps1`. The rendered `card-website` is captured by the existing full-page `screenshotPage`, so the snapshot appears in the exported PNG automatically. |
