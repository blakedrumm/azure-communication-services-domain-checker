...
| File | Lines | Contents |
|---|---|---|
| `03-MetricsHashKey.ps1` | ~117 | `$script:AppVersion` (currently `2.4.1`), metrics hash key persistence, `Get-HashedDomain`, `Handle-MetricsRequest` |
...
| File | Lines | Contents |
|---|---|---|
| `14-SpfAnalysis.ps1` | ~1,100 | Deep SPF record analysis — recursive expansion, Outlook requirement matching, guidance generation. The Outlook requirement check has four detection paths: (1) literal `include:spf.protection.outlook.com` in the queried-domain TXT, (2) nested-include match in the expanded chain (`Find-SpfOutlookRequirementMatch`), (3) `redirect=` target match, and (4) **flattened-include** — when the customer uses an SPF-flattening / Dynamic SPF service (OnDMARC, Valimail, Sendmarc, EasyDMARC, Sparkpost, etc.) the literal include is stripped and the upstream EOP IP ranges are inlined as `ip4:`/`ip6:` tokens. To still recognize this case `Get-OutlookSpfCanonicalRanges` resolves `spf.protection.outlook.com` live (recursing into any nested includes it publishes), parses every `ip4:`/`ip6:` CIDR it currently advertises, and caches the result in `$script:OutlookSpfCanonicalCache` for ~24h. `Test-SpfChainCoversOutlookRanges` then walks the customer's analysis tree, collects every authorized `ip4:` CIDR, and reports `isCovered = $true` when every canonical IPv4 EOP range is either fully contained in a single authorized range or covered by the tiled union of authorized sub-ranges (BigInteger subnet math via `ConvertTo-Ipv4CidrRange` / `ConvertTo-Ipv6CidrRange`). When the coverage check passes `Get-SpfOutlookRequirementStatus` returns `matchType = 'flattened-include'` with a detail string explaining that the upstream EOP ranges were detected inline. Canonical ranges are **never hard-coded** — they always come from the live `spf.protection.outlook.com` TXT record so the check stays correct as Microsoft updates its published ranges. |
...
| `20d-HtmlJsCore.ps1` | ~1,385 | Core UI JavaScript: theme toggle, clipboard/copy, screenshot capture, `lookup()`, `render()`, `card()`, event listeners, initialization. `render()` prepends a **Jump to Section** navigation card (`card-section-nav`, hidden from screenshots) built by `buildSectionNavHtml(markup)`, which uses the shared `parseSectionNavEntries(markup)` helper to regex-parse the assembled card markup for `id="card-KEY"` + the card's status `<span class="tag ...">` badge + `<strong>` title so the nav always matches the cards that actually rendered, in their on-page order; each button shows a colored status dot and (for real pass/warn/fail) the status word, and wires to `scrollToSection(key)`. `render()` also calls `scheduleSectionRailBuild(markup)` → `buildSectionRail(markup)` to populate the **floating left-hand section rail** (`#sectionRail`, declared in `20a-HtmlScriptSetup.ps1`, `position:fixed` on viewports ≥1500px, hidden from screenshots/narrow screens). The rail is a scrollspy: an `IntersectionObserver` (rootMargin `-20% 0px -55% 0px`) tracks each card's visibility ratio and `updateActiveSectionRailLink()` highlights the dominant in-view section's link (`.section-rail-link-active`). For both navs every result card must expose a `card-...` id and a `<strong>` title — the Guidance (`card-guidance`), Helpful Links (`card-helpfulLinks`), and External Tools (`card-tools`) cards were given ids specifically so they appear. Translation keys: `jumpToSection`, `jumpToTag`. CSS: `.section-nav-*` and `.section-rail-*` in `20-HtmlCss.ps1`. |
