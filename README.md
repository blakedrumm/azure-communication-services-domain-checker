...
Mail provider detection recognizes Microsoft 365, Azure Communication Services Email (`xm.communication.azure.com` / `azurecomm.net`), Google Workspace, Zoho, Proofpoint, Mimecast, and Cloudflare Email Routing out of the box.
...
The SPF, ACS Domain Verification, and TXT Records cards are driven by a single public DNS-over-HTTPS resolver. When a domain's authoritative DNS answers unreliably — either an **intermittent SERVFAIL/timeout** or **nameservers that are out of sync** — that resolver can return an empty answer for a lookup, so those cards would show "No Records Available" even though the records exist (and resolve fine on the next attempt). To stop the tool from contradicting itself, the nameserver check doubles as a **recovery source**: when the normal lookup returns no queried-domain TXT but the authoritative nameservers do have them, the cards backfill from the union of records across the responding nameservers. Recovered records are rendered as **WARN** (never PASS) with a note explaining the record exists on the nameservers but is not resolving reliably via public DNS — which is what governs real email delivery and Azure domain verification. The note deliberately does **not** assert that the nameservers are inconsistent (the Nameserver TXT Consistency card may well report them as consistent if it happened to query them during a healthy moment); it points the operator at that card to confirm whether the cause is a transient authoritative failure or genuinely divergent record sets.
